...
class GuestService {
	async registerGuest(req, res) {
		...
		if (!guestInstance.firstName){
			return ResponseService.sendResponse(res,400,"BAD REQUEST","FAILURE","first name missing");
		} else if (!guestInstance.lastName){
			return ResponseService.sendResponse(res,400,"BAD REQUEST","FAILURE","last name missing");
		} else if (!guestInstance.email){
			return ResponseService.sendResponse(res,400,"BAD REQUEST","FAILURE","email missing");
		} else if (!guestInstance.dateOfBirth){
			return ResponseService.sendResponse(res,400,"BAD REQUEST","FAILURE","email missing");
		} else if (!guestInstance.nationalIdNumber){
			return ResponseService.sendResponse(res,400,"BAD REQUEST","FAILURE","national id number missing");
		}
